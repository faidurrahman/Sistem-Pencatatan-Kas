  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success || result.status === 'success') {
        ...
        setFormData({ username: '', password_lama: '', password_baru: '' });
      } else {
        setStatus('error');
  ...
  return (
    <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6 sm:p-8 w-full max-w-lg mx-auto">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-extrabold text-gray-900 tracking-tight">Ubah Password</h2>
          <p className="text-sm text-gray-500 font-medium">Perbarui password akun Anda</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 text-gray-900 text-sm font-semibold rounded-xl focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 outline-none transition-all"
        ...
        <button 
          ...
          className="w-full flex items-center justify-center space-x-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3.5 rounded-xl font-bold transition-all shadow-md hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed mt-2"
        >
          {status === 'loading' ? (
